...
document.getElementById('roomFilter').addEventListener('change', applyFiltersAndSort);
...
function populateFilters(exams) {
    ...
    const uniqueDates = [...new Set(exams.map(e => e.exam_date).filter(Boolean))].sort();
    const uniqueRooms = [...new Set(exams.map(e => e.room).filter(Boolean))].sort();
...
function applyFiltersAndSort() {
    ...
    if (currentSort.column && currentSort.direction !== 'none') {
        filteredExams.sort((a, b) => {
            ...
            let comparison = 0;
            if (currentSort.column === 'exam_date' || currentSort.column === 'start_time') {
    ...
    if (exams.length === 0) {
        ...
        rows.forEach((row, index) => {
            row.addEventListener('click', () => {
                openCalendarModal(exams[index]); // Pass the actual exam object from the filtered/sorted array
            });
        });
    }
}
